...
import { ProBillItemsTable } from '@/components/propos/ProBillItemsTable';
import { ProBusinessDetails } from '@/components/propos/ProBusinessDetails';
import { ProCustomerDetails } from '@/components/propos/ProCustomerDetails';
import { ProCustomerSearchDialog } from '@/components/propos/ProCustomerSearchDialog';
import { ProHeaderSection } from '@/components/propos/ProHeaderSection';
import { ProInvoiceSummary } from '@/components/propos/ProInvoiceSummary';
import { ProNetworkStatus } from '@/components/propos/ProNetworkStatus';
import { ProProductSearchDialog } from '@/components/propos/ProProductSearchDialog';
import { ProSubscriptionStatus } from '@/components/propos/ProSubscriptionStatus';
import { proColors } from '@/components/propos/types';
...
export default function ProPOSPage() {
  ...
  const selectCustomer = (cust: Customer) => {
    setCustomer({
      ...
      gstin: cust.gstin || '',
      isInterState: business?.state ? cust.state !== business.state : false,
    });
  ...
  const handleConfirmBill = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        window.open(`/invoices/${data.invoiceId}/print`, '_blank');
        setItems([]);
        setCustomer({ name: '', phone: '', email: '', company: '', address: '', city: '', state: '', pincode: '', gstin: '', isInterState: false });
        setNotes('');
      }
    } catch (error) {
  ...
  const handleSyncOffline = async () => {
    // Sync offline bills logic
    console.log('Syncing offline bills...');
  };

  return (
    <MainLayout title="Point of Sale">
      <Box sx={{ minHeight: '100vh', backgroundColor: darkMode ? proColors.bgDark : proColors.bgLight, py: { xs: 2, sm: 3 } }}>
        <Container maxWidth="xl">
          <ProNetworkStatus isOnline={isOnline} offlineBillsCount={offlineBillsCount} onSyncClick={handleSyncOffline} subscription={subscription} usage={usage} />
          <ProSubscriptionStatus isLoading={false} isActive={subscription?.isActive} isOnline={isOnline} remainingInvoices={subscription?.limits?.invoices - (usage?.invoices || 0)} />
          <ProHeaderSection isOnline={isOnline} grandTotal={grandTotal} />
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} lg={8}>
              <ProBillItemsTable
                ...
                onAddProduct={() => setProductSearchOpen(true)}
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  <ProBusinessDetails business={business} />
                </Grid>
                <Grid item xs={12} md={6}>
                  <ProCustomerDetails
                    customer={customer}
                    businessState={business?.state}
                    onCustomerChange={(field, value) => setCustomer(prev => ({ ...prev, [field]: value, isInterState: business?.state ? value !== business.state : false }))}
