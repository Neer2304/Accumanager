...
const SalesChart: React.FC<SalesChartProps> = ({
  ...
  const fetchSalesData = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(
          `Error ${response.status}: ${errorText || "Failed to load data"}`,
        );
      }
    } catch (error) {
  ...
  const handleExportData = () => {
    ...
    const csvContent =
      ...
      data
        .map(
          (d) =>
            `${d.date},${d.orders},${d.revenue},${d.totalItems},${d.avgOrderValue || 0}`,
        )
        .join("\n");
    ...
    link.setAttribute(
      "download",
      `sales_data_${timeRange}_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const totalItems = data.reduce(
    (sum, item) => sum + ((item as any).totalItems || 0),
    0,
  );
  ...
  const weeklyBreakdown =
    data.length > 0
      ? [
          ...
          { name: "Fri", value: Math.round(totalRevenue * 0.2) },
    ...
    >
      ...
      >
        ...
        >
          ...
          {formatCompactNumber(
            Math.round(totalRevenue / Math.max(data.length, 1)),
          )}{" "}
        </Typography>
      </Paper>
      <Paper
      ...
      >
        ...
        >
          ...
          {data.length > 0
            ? formatCompactNumber(Math.max(...data.map((d) => d.revenue)))
            : "₹0"}
        </Typography>
      </Paper>
      <Paper
      ...
      >
        ...
        >
          {totalItems > 0
            ? ((totalOrders / totalItems) * 100).toFixed(1) + "%"
            : "0%"}
        </Typography>
      </Paper>
    </Box>
          ...
          <ResponsiveContainer width="100%" height={chartHeight}>
            ...
            >
              <CartesianGrid
                strokeDasharray="3 3"
                stroke={
                  darkMode ? alpha("#3c4043", 0.5) : alpha("#dadce0", 0.5)
                }
                vertical={false}
              />
              <XAxis
              ...
              {weeklyBreakdown.map((item, index) => (
                <Box
                  ...
                  sx={{
                    ...
                    borderRadius: 1,
                    backgroundColor: darkMode
                      ? alpha(COLORS[index % COLORS.length], 0.1)
                      : alpha(COLORS[index % COLORS.length], 0.05),
      ...
      >
        ...
        >
          ...
          >
            ...
            <Tab
              icon={
                <CompareArrowsIcon fontSize={isMobile ? "small" : "medium"} />
              }
