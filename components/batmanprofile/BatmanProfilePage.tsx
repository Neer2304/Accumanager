// components/batmanprofile/BatmanProfilePage.tsx
'use client';
...
import {
  BatmanProfileSkeleton,
  BatmanProfileHeader,
  BatmanProfileStats,
  BatmanProfileTabs,
  BatmanProfilePersonal,
  BatmanProfileBusiness,
  BatmanProfileNotifications,
  BatmanProfileSecurity,
  BatmanProfileSubscription,
  BatmanProfileUpgradeDialog,
  UserProfile,
  BusinessData,
  SubscriptionStatus,
  ProfileFormData,
...
export default function BatmanProfilePage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSnackbar({ open: true, message: 'Profile updated! Justice is served. 🦇', severity: 'success' });
  ...
  const handleUpdateBusiness = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.business) {
          setBusinessData(data.business);
          setSnackbar({ open: true, message: 'Business details updated! Wayne Enterprises approves. 🏢', severity: 'success' });
        }
      } else if (response.status === 401) {
        logout();
      } else {
  ...
  const handlePreferenceChange = async (preference: keyof UserProfile['preferences'], value: boolean) => {
    try {
      ...
      if (response.ok) {
        const updatedProfile = await response.json();
        setProfile(updatedProfile);
        setSnackbar({ open: true, message: 'Preferences updated! Batcomputer synced. 🦇', severity: 'success' });
      } else if (response.status === 401) {
        logout();
      } else {
        const errorData = await response.json();
  ...
  const handleChangePassword = async () => {
    if (passwords.newPassword !== passwords.confirmPassword) {
      setSnackbar({ open: true, message: 'Passwords do not match! Secure your identity!', severity: 'error' });
      return;
    }
    if (passwords.newPassword.length < 6) {
      setSnackbar({ open: true, message: 'Password must be at least 6 characters! Strength is key.', severity: 'error' });
      return;
    }
    setIsSaving(true);
    try {
      ...
      if (response.ok) {
        setSnackbar({ open: true, message: 'Password changed! Your secret identity is safe. 🦇', severity: 'success' });
  ...
  const handleUpgradePlan = async (plan: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setSnackbar({ open: true, message: `Payment initiated for ${PRICING_PLANS[plan].name}! The Batcomputer approves. 🦇`, severity: 'success' });
    ...
    return (
      <MainLayout title="Profile">
        <Container maxWidth="lg" sx={{ py: 4 }}>
          <Alert severity="error" sx={{ borderRadius: '12px', border: `2px solid ${batmanColors.error}` }}>Please log in to access the Batcomputer. 🦇</Alert>
        </Container>
      </MainLayout>
    );
  }
    ...
    <MainLayout title="Profile">
      <Box sx={{ minHeight: '100vh', backgroundColor: batmanColors.bg }}>
          ...
          <Box sx={{ px: { xs: 1, sm: 2, md: 3 }, pt: 2 }}>
            <Fade in>
              <Alert
                ...
                sx={{ borderRadius: '12px', backgroundColor: batmanColors.surface, border: `2px solid ${snackbar.severity === 'success' ? '#00ff88' : batmanColors.error}`, color: batmanColors.ink }}
              >
                {snackbar.severity === 'success' ? '🦇 ' : '⚠️ '}{snackbar.message}
              </Alert>
            </Fade>
          </Box>
        )}
        ...
        <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
          <BatmanProfileTabs activeTab={activeTab} onTabChange={setActiveTab} isMobile={isMobile} darkMode={darkMode}>
