// components/batmanprofile/types.ts
...
export const getPlanColor = (plan: string): string => {
  switch (plan) { case 'trial': return batmanColors.inkSub; case 'monthly': return batmanColors.gold; case 'quarterly': return batmanColors.gold; case 'yearly': return batmanColors.gold; default: return batmanColors.inkSub; }
};

export const getProgressColor = (percentage: number): string => {
  if (percentage > 90) return batmanColors.error;
  if (percentage > 75) return batmanColors.gold;
  return '#00ff88';
};