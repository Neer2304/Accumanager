// components/batmanpos/BatmanPOSPage.tsx
'use client';
...
import { BatmanBillItemsTable } from './components/BatmanBillItemsTable';
import { BatmanBusinessDetails } from './components/BatmanBusinessDetails';
import { BatmanCustomerDetails } from './components/BatmanCustomerDetails';
import { BatmanCustomerSearchDialog } from './components/BatmanCustomerSearchDialog';
import { BatmanHeaderSection } from './components/BatmanHeaderSection';
import { BatmanInvoiceSummary } from './components/BatmanInvoiceSummary';
import { BatmanNetworkStatus } from './components/BatmanNetworkStatus';
import { BatmanProductSearchDialog } from './components/BatmanProductSearchDialog';
import { BatmanSubscriptionStatus } from './components/BatmanSubscriptionStatus';
...
export default function BatmanPOSPage() {
  ...
  const handleSaveDraft = async () => { console.log('Saving draft to Batcomputer...'); };
  const handleSyncOffline = async () => { console.log('Syncing offline bills to Batcomputer...'); };

  return (
    <MainLayout title="Point of Sale - Batcomputer">
      <Box sx={{ minHeight: '100vh', backgroundColor: batmanColors.bg, py: { xs: 2, sm: 3 } }}>
        <Container maxWidth="xl">
          <BatmanNetworkStatus isOnline={isOnline} offlineBillsCount={offlineBillsCount} onSyncClick={handleSyncOffline} subscription={subscription} usage={usage} />
          <BatmanSubscriptionStatus isLoading={false} isActive={subscription?.isActive} isOnline={isOnline} remainingInvoices={(subscription?.limits?.invoices || 0) - (usage?.invoices || 0)} />
          <BatmanHeaderSection isOnline={isOnline} grandTotal={grandTotal} />
          
          <Grid container spacing={3}>
            <Grid item xs={12} lg={8}>
              <BatmanBillItemsTable items={items} isInterState={isInterState} onAddProduct={() => setProductSearchOpen(true)} onUpdateItem={updateItem} onRemoveItem={removeItem} isOnline={isOnline} isSubscriptionActive={subscription?.isActive} />
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}><BatmanBusinessDetails business={business} /></Grid>
                <Grid item xs={12} md={6}><BatmanCustomerDetails customer={customer} businessState={business?.state} onCustomerChange={(field, value) => setCustomer(prev => ({ ...prev, [field]: value, isInterState: business?.state ? value !== business.state : false }))} onOpenSearch={() => setCustomerSearchOpen(true)} /></Grid>
              </Grid>
            </Grid>
            <Grid item xs={12} lg={4}>
              <BatmanInvoiceSummary
      ...
      <BatmanProductSearchDialog open={productSearchOpen} searchTerm={searchTerm} searchResults={searchResults} isInterState={isInterState} onClose={() => setProductSearchOpen(false)} onSearchChange={searchProducts} onSelectProduct={addProduct} />
      <BatmanCustomerSearchDialog open={customerSearchOpen} searchTerm={customerSearchTerm} searchResults={customerSearchResults} onClose={() => setCustomerSearchOpen(false)} onSearchChange={searchCustomers} onSelectCustomer={selectCustomer} />
    </MainLayout>
  );
}