// components/proprofile/ProProfilePage.tsx
'use client';
...
import {
  ...
  ProProfilePersonal,
  ProProfileBusiness,
  ProProfileNotifications,
  ProProfileSecurity,
  ProProfileSubscription,
  ProProfileUpgradeDialog,
  UserProfile,
  BusinessData,
  SubscriptionStatus,
  ProfileFormData,
  ...
  const handleUpdateBusiness = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.business) {
          setBusinessData(data.business);
          setSnackbar({ open: true, message: 'Business details updated successfully', severity: 'success' });
        }
      } else if (response.status === 401) {
        logout();
      } else {
  ...
  const handleUpgradePlan = async (plan: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setSnackbar({ open: true, message: `Payment initiated for ${PRICING_PLANS[plan].name}. Please complete payment.`, severity: 'success' });
    ...
    return (
      <MainLayout title="Profile">
        <Container maxWidth="lg" sx={{ py: 4 }}>
          <Alert severity="error" sx={{ borderRadius: '12px' }}>Please log in to view your profile.</Alert>
        </Container>
      </MainLayout>
    );
  }
    ...
    <MainLayout title="Profile">
      <Box sx={{ minHeight: '100vh', backgroundColor: darkMode ? '#202124' : '#f8f9fa' }}>
        ...
        <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
          <ProProfileTabs activeTab={activeTab} onTabChange={setActiveTab} isMobile={isMobile} darkMode={darkMode}>
