// app/api/admin/legal/shipping-policy/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const document = await LegalDocument.findOne({ 
      type: 'shipping_policy'  // Database uses underscores
...
export async function PUT(request: NextRequest) {
  try {
    ...
    let document = await LegalDocument.findOne({ type: 'shipping_policy' });
    ...
    return NextResponse.json({
      success: true,
      message: document.isNew ? 'Shipping Policy created' : 'Shipping Policy updated',
      data: document
    });
