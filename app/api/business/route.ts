...
import Business from '@/models/Bussiness' // ✅ Fixed import
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!authToken) {
      console.log('❌ No auth token in business update request')
      return NextResponse.json({ message: 'Unauthorized' }, { status: 401 })
    }
    ...
    try {
      ...
      await connectToDatabase()
      console.log('✅ Database connected for business update')

      const businessData = await request.json()
      console.log('📦 Received update data:', businessData)
      ...
      console.error('❌ Auth error in business update:', authError)
      return NextResponse.json({ message: 'Invalid token' }, { status: 401 })
    }
  } catch (error: any) {
