    ...
    try {
      ...
      if (!subscription.isActive) {
        return NextResponse.json(
          { 
            success: false,
            error: 'Project management requires an active subscription. Please upgrade your plan.' 
          },
          { status: 402 }
        );
      }
      ...
      console.log(`✅ Found ${projects.length} projects for user ${decoded.userId}`);
    ...
    try {
      ...
      if (!subscription.isActive) {
        return NextResponse.json(
          { 
            success: false,
            error: 'Project creation requires an active subscription. Please upgrade your plan.' 
          },
          { status: 402 }
        );
      }
      ...
      await connectToDatabase();
      const userProjectCount = await Project.countDocuments({ userId: decoded.userId });
      
      if (subscription.limits?.projects && userProjectCount >= subscription.limits.projects) {
        return NextResponse.json(
          { 
            success: false,
      ...
      if (!projectData.name || !projectData.description || !projectData.startDate || !projectData.deadline) {
        return NextResponse.json(
          { 
            success: false,
      ...
      try {
        ...
        console.error('⚠️ Failed to create notification (project still saved):', notifError);
      }
      ...
      if (updateData.status === 'completed') {
        try {
          const notification = new Notification({
            ...
            message: `Congratulations! Project "${project.name}" has been completed successfully.`,
            type: 'success',
